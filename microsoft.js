...
import { delay, datetime, prompt, notify, log, dataDir, jsonDb, clearBrowserLock } from './src/util.js';
...
log.section('Desktop');
{
  ...
  sessionResults.desktop = { before, after };
...
log.section('Mobile');
{
  ...
  sessionResults.mobile = { before, after };
  await recordMsRun('mobile', startedAt, before, after);
}

{
  const fmt = (s, r) => {
    if (r.after == null) return `${s}: no data`;
    const earned = (r.before != null && r.after != null) ? ` (+${r.after - r.before})` : '';
    return `${s}: ${r.after}${earned} pts`;
  };
  const d = fmt('desktop', sessionResults.desktop);
  const m = fmt('mobile', sessionResults.mobile);
  const totalAfter = [sessionResults.desktop.after, sessionResults.mobile.after].filter(v => v != null).pop();
  const totalEarned = [sessionResults.desktop, sessionResults.mobile]
    .filter(r => r.before != null && r.after != null)
    .reduce((sum, r) => sum + (r.after - r.before), 0);
  const summary = totalEarned > 0 ? ` | total +${totalEarned} pts` : '';
  await notify(`microsoft-rewards: done<br>${d}<br>${m}${summary}`);
}
log.section('Done');
log.status('Time', datetime());
