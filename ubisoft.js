...
for (const match of html.matchAll(/data-tc100="([^"]*)"/g)) {
...
// Only notify for non-F2P entries ("Free Access", "Free for Limited Time", etc.)
...
const subject = `Ubisoft — ${promoEntries.length} new free game${promoEntries.length === 1 ? '' : 's'} — claim manually`;
log.info(subject);
